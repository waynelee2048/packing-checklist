...
export default function AdminSharing({ data, shared }) {
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-slate-900 dark:text-slate-50 mb-4">分享管理</h2>

      {/* Tabs */}
      <div className="flex border-b border-slate-200 dark:border-slate-700 mb-4">
        <button
          onClick={() => setTab('byMe')}
          className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 transition-colors ${tab === 'byMe' ? 'border-indigo-600 text-indigo-600 dark:border-indigo-400 dark:text-indigo-400' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
        >
          <Share2 size={16} /> 我分享的 ({byMeEntries.length})
        </button>
        <button
          onClick={() => setTab('withMe')}
          className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 transition-colors ${tab === 'withMe' ? 'border-indigo-600 text-indigo-600 dark:border-indigo-400 dark:text-indigo-400' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
        >
            ...
            <>
              ...
              <div className="hidden lg:block bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                <table className="w-full">
                  <thead className="bg-slate-50 dark:bg-slate-700 border-b border-slate-200 dark:border-slate-600">
                    <tr>
                      <th className="text-left px-4 py-3 text-sm font-semibold text-slate-600 dark:text-slate-300">清單名稱</th>
                      <th className="text-left px-4 py-3 text-sm font-semibold text-slate-600 dark:text-slate-300">分享對象</th>
                      <th className="w-32 px-4 py-3 text-sm font-semibold text-slate-600 dark:text-slate-300">操作</th>
                    </tr>
                  </thead>
                  <tbody>
                    {byMeEntries.map(([id, sharedList]) => {
                      ...
                      return (
                        <tr key={id} className="border-b border-slate-100 dark:border-slate-700 last:border-0">
                          <td className="px-4 py-3 text-sm font-medium text-slate-800 dark:text-slate-100">{getListName(id)}</td>
                          <td className="px-4 py-3">
                            <div className="flex flex-wrap gap-1.5">
                              {emails.map(email => (
                                <span key={email} className="inline-flex items-center gap-1 px-2 py-0.5 text-xs bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-full">
                                  {email}
                              ...
                              {addingEmail[id] ? (
                                ...
                                <button onClick={() => setAddingEmail({ [id]: true })} className="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">+ 新增</button>
                              )}
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <button onClick={() => handleUnshare(id)} className="flex items-center gap-1 px-2 py-1 text-xs text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-lg">
                              <Trash2 size={14} /> 取消分享
                            </button>
                          </td>
                        </tr>
              ...
              <div className="lg:hidden space-y-3">
                {byMeEntries.map(([id, sharedList]) => {
                  const emails = Object.keys(sharedList.sharedWith || {}).map(decodeEmail);
                  return (
                    <div key={id} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
                      <div className="flex items-center justify-between mb-2">
                        <h3 className="font-medium text-slate-800 dark:text-slate-100">{getListName(id)}</h3>
                      ...
                      {addingEmail[id] ? (
                        ...
                        <button onClick={() => setAddingEmail({ [id]: true })} className="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">+ 新增分享對象</button>
                      )}
                    </div>
                  );
                })}
            ...
            <>
              ...
              <div className="hidden lg:block bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                <table className="w-full">
                  <thead className="bg-slate-50 dark:bg-slate-700 border-b border-slate-200 dark:border-slate-600">
                    <tr>
                      <th className="text-left px-4 py-3 text-sm font-semibold text-slate-600 dark:text-slate-300">清單名稱</th>
                      <th className="text-left px-4 py-3 text-sm font-semibold text-slate-600 dark:text-slate-300">來源</th>
                      <th className="text-left px-4 py-3 text-sm font-semibold text-slate-600 dark:text-slate-300">勾選進度</th>
                    </tr>
                  </thead>
                  <tbody>
                    {withMeEntries.map(([id, sharedList]) => {
                      ...
                      return (
                        <tr key={id} className="border-b border-slate-100 dark:border-slate-700 last:border-0">
                          <td className="px-4 py-3 text-sm font-medium text-slate-800 dark:text-slate-100">{sharedList.name}</td>
                          <td className="px-4 py-3 text-sm text-slate-500 dark:text-slate-400">{sharedList.ownerName || sharedList.ownerEmail}</td>
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <div className="w-24 h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                                <div className="h-full bg-emerald-500 rounded-full" style={{ width: total > 0 ? `${(checked / total) * 100}%` : '0%' }} />
                              </div>
                              <span className="text-xs text-slate-500 dark:text-slate-400">{checked}/{total}</span>
                            </div>
                          </td>
                        </tr>
                      );
              ...
              <div className="lg:hidden space-y-3">
                {withMeEntries.map(([id, sharedList]) => {
                  ...
                  return (
                    <div key={id} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
                      <h3 className="font-medium text-slate-800 dark:text-slate-100">{sharedList.name}</h3>
                      <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">來自 {sharedList.ownerName || sharedList.ownerEmail}</p>
