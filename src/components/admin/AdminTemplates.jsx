  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-slate-900">清單範本</h2>
        {!showCreate && (
      ...
      {showCreate && (
        <div className="bg-white border border-slate-200 rounded-xl p-4 mb-4">
          <h3 className="font-semibold text-slate-900 mb-3">{editingId ? '編輯範本' : '建立範本'}</h3>

          {!editingId && createMode === 'list' && (
            <div className="mb-4">
              <label className="block text-sm text-slate-600 mb-1">從現有清單建立</label>
              <div className="flex gap-2">
                <select value={selectedListId} onChange={e => setSelectedListId(e.target.value)} className="flex-1 px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white">
              ...
              <button onClick={() => setCreateMode('manual')} className="mt-2 text-sm text-indigo-600 hover:underline">或手動挑選物品</button>
            </div>
          )}
          ...
          {(createMode === 'manual' || editingId) && (
            <div className="space-y-3">
              <div>
                <label className="block text-sm text-slate-600 mb-1">範本名稱</label>
                <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm" placeholder="例：出國旅行" />
              </div>
              <div>
                <label className="block text-sm text-slate-600 mb-1">圖示</label>
                <select value={form.icon} onChange={e => setForm(f => ({ ...f, icon: e.target.value }))} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white">
                  {iconOptions.map(icon => <option key={icon} value={icon}>{icon}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm text-slate-600 mb-1">說明</label>
                <input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm" placeholder="選填" />
              </div>
              <div>
                <label className="block text-sm text-slate-600 mb-1">選擇物品 ({form.itemIds.length})</label>
                <div className="max-h-48 overflow-y-auto border border-slate-200 rounded-lg">
                  {data.itemLibrary.map(item => (
                    <label key={item.id} className="flex items-center gap-2 px-3 py-2 hover:bg-slate-50 cursor-pointer border-b border-slate-100 last:border-0">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { setEditingId(null); resetForm(); }} className="px-4 py-2 text-sm bg-slate-200 rounded-lg hover:bg-slate-300">取消</button>
              </div>
            </div>
          )}
        </div>
      ...
      {applyingId && (
        <div className="bg-white border border-indigo-200 rounded-xl p-4 mb-4">
          <h3 className="font-semibold text-slate-900 mb-2">套用範本</h3>
          <p className="text-sm text-slate-600 mb-3">選擇要套用的目標清單：</p>
          <div className="flex gap-2">
            <select value={targetListId} onChange={e => setTargetListId(e.target.value)} className="flex-1 px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white">
              <option value="">選擇清單...</option>
              {data.lists.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}
            </select>
            ...
            <button onClick={() => { setApplyingId(null); setTargetListId(''); }} className="px-3 py-2 text-sm bg-slate-200 rounded-lg hover:bg-slate-300">
              <X size={16} />
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {templates.map(template => (
          <div key={template.id} className="bg-white border border-slate-200 rounded-xl p-4">
            <div className="flex items-start justify-between mb-2">
              <div>
                <h3 className="font-semibold text-slate-900">{template.name}</h3>
                {template.description && <p className="text-sm text-slate-500 mt-0.5">{template.description}</p>}
              </div>
              <span className="text-xs text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">{template.icon}</span>
            </div>
            <div className="flex flex-wrap gap-1 mb-3">
              {getItemNames(template.itemIds).slice(0, 5).map((name, i) => (
                <span key={i} className="text-xs bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full">{name}</span>
              ))}
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm text-slate-500"><Package size={14} className="inline mr-1" />{template.itemIds.length} 個物品</span>
              <div className="flex gap-1">
                <button onClick={() => setApplyingId(template.id)} className="px-2.5 py-1 text-xs bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100">套用</button>
                <button onClick={() => startEdit(template)} className="p-1.5 text-slate-400 hover:text-indigo-600"><Edit3 size={14} /></button>
